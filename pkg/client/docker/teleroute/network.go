...
func CreateNetwork(ctx context.Context, id *daemon.Identifier, cli *dockerClient.Client, teleroutePlugin string, teleroutePort int) error {
	ci, err := cli.ContainerInspect(ctx, id.ContainerName())
	if err != nil {
		return fmt.Errorf("docker container inspect %s: %w", id.ContainerName(), err)
	}
	...
	if ns := ci.NetworkSettings; ns != nil {
		if tn, ok := ns.Networks["bridge"]; ok {
			address = tn.IPAddress
			pid = ci.State.Pid
		}
	}
	if address == "" {
		return fmt.Errorf("failed to retrieve IP for container %s", id.ContainerName())
	}
	...
	rsp, err := cli.NetworkCreate(ctx, cn, network.CreateOptions{
		...
		Options: map[string]string{
			"host": address,
			"port": strconv.Itoa(teleroutePort),
