...
func ContainerPidAndIP(ctx context.Context, name string) (int, string, error) {
	cli, err := GetClient(ctx)
	if err != nil {
		return 0, "", err
	}
	...
	if err != nil {
		return 0, "", fmt.Errorf("docker container inspect %s: %w", "userd", err)
	}
	if ns := ci.NetworkSettings; ns != nil {
		if tn, ok := ns.Networks["bridge"]; ok {
			return ci.State.Pid, tn.IPAddress, nil
		}
	}
